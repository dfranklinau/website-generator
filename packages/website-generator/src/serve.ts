...
// @ts-expect-error: Parameter 'resource' implicitly has an 'any' type.
const fetchResource = async (resource) => {
  const pathname = path.join(ROOT, resource);
...
const httpServer = http.createServer(async (request, response) => {
  ...
  try {
    ...
    if (resource.pathname.endsWith(".html")) {
      resource.body = Buffer.from(resource.body.toString().replace("</body>", '<script src="/client.js"></script>'));
    }
