...
export function serve() {
  ...
  // Start file watching and call `generate` whenever files change; then send a
  // reload event to the WebSocket server after `generate` is successful.
  chokidar
    .watch(`${CONTENT}/**/*`, {
      ignoreInitial: true,
    })
    .on('all', (event, path) => {
      generate().then(() => wsServer.emit('reload'))
    });
}
